...
const properties = [
  ...
  { camelCase: 'viewSubscriptionPrices', raw: 'VIEW_SUBSCRIPTION_PLANS', description: 'Allow users to browse published.prices', value: 'true', noDefaultValue: true, valueDescription: 'Boolean' }
]

describe('index', () => {
  const webhookSecret = global.subscriptionWebhookEndPointSecret
  const stripeKey = global.stripeKey
  const stripePublishableKey = global.stripePublishableKey
  before(async () => {
