...
describe('/administrator/subscriptions/unpublish-price', function () {
  describe('before', () => {
    it('should bind data to req', async () => {
      const administrator = await TestStripeAccounts.createOwnerWithPrice()
      const req = TestHelper.createRequest(`/administrator/subscriptions/unpublish-price?priceid=${administrator.price.priceid}`)
  ...
  describe('submit', () => {
    it('should unpublish price (screenshots)', async () => {
      const administrator = await TestStripeAccounts.createOwnerWithPrice()
      const req = TestHelper.createRequest(`/administrator/subscriptions/unpublish-price?priceid=${administrator.price.priceid}`)
      req.account = administrator.account
      req.session = administrator.session
      ...
      req.screenshots = [
        ...
        { click: `/administrator/subscriptions/unpublish-price?priceid=${administrator.price.priceid}` },
        { fill: '#submit-form' }
      ]
      global.pageSize = 50
  ...
  describe('errors', () => {
    it('invalid-priceid', async () => {
      ...
      const req = TestHelper.createRequest('/administrator/subscriptions/unpublish-price?priceid=invalid')
      req.account = administrator.account
      req.session = administrator.session
      await req.route.api.before(req)
