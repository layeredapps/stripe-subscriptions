...
describe('/administrator/subscriptions/adjust-subscription-item-quantity', function () {
  describe('before', () => {
    it('should bind data to req', async () => {
      const administrator = await TestStripeAccounts.createOwnerWithPrice()
      const user = await TestStripeAccounts.createUserWithPaidSubscription(administrator.price)
      const req = TestHelper.createRequest(`/administrator/subscriptions/adjust-subscription-item-quantity?subscriptionitemid=${user.subscription.stripeObject.items.data[0].id}`)
  ...
  describe('submit', () => {
    it('should adjust quantity (screenshots)', async () => {
      const administrator = await TestStripeAccounts.createOwnerWithPrice()
      const user = await TestStripeAccounts.createUserWithPaidSubscription(administrator.price)
      const req = TestHelper.createRequest(`/administrator/subscriptions/adjust-subscription-item-quantity?subscriptionitemid=${user.subscription.stripeObject.items.data[0].id}`)
      req.account = administrator.account
      ...
      req.screenshots = [
        ...
        { click: `/administrator/subscriptions/adjust-subscription-item-quantity?subscriptionitemid=${user.subscription.stripeObject.items.data[0].id}` },
        { fill: '#submit-form' }
      ]
      global.pageSize = 50
  ...
  describe('errors', () => {
    it('invalid-subscriptionitemid', async () => {
      ...
      const req = TestHelper.createRequest('/administrator/subscriptions/adjust-subscription-item-quantity?subscriptionitemid=invalid')
      req.account = administrator.account
      req.session = administrator.session
      req.body = {
        quantity: '7'
